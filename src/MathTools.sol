...
library CoreTools {
    uint256 constant MAX_ODDS = ONE * 100;
    ...
    function marginAdjustedOdds(
        ...
        for (uint256 i = 0; i < length; ++i) {
            spreads[i] = (ONE - probabilities[i]).mul(margin);
        }

        uint256 error = margin;
        uint256 iteration;
        uint256 spreadMultiplier = ONE;
        for (; iteration < MAX_ITERATIONS; ++iteration) {
            uint256 oddsSpread;
            {
                uint256 spread;
                for (uint256 i = 0; i < length; ++i) {
                    uint256 odds_ = (ONE - spreads[i]).div(
                        probabilities[i]
                    );
                    odds[i] = odds_;
                    spread += ONE.div(odds_);
                }

                oddsSpread = ONE - spreadMultiplier.div(spread);
            }

            if (FixedMath.ratio(margin, oddsSpread) - ONE < PRECISION)
                break;
            assert(margin > oddsSpread);

            uint256 newError = margin - oddsSpread;
            if (newError == error) {
                assert(margin.div(oddsSpread) - ONE < 1e9); // Raise an assertion error if the difference between the expected and actual margin is greater than 0.1%
                break;
            }
            ...
            for (uint256 i = 0; i < length; ++i)
                spreads[i] += (ONE - spreads[i] - probabilities[i])
                    .mul(
                        FixedMath.sigmoid(
                            (error * spreads[i])
                                .div(ONE - ONE.div(odds[i]))
                                .div(ONE - margin) / (oddsSpread)
                        )
                    );
        }
    ...
    function calcOdds(
        ...
        if (margin > 0) {
            ...
            for (uint256 i = 0; i < length; ++i) {
                ...
                if (odds_ <= ONE) revert IncorrectOdds();
    ...
    function calcProbability(
        ...
        if (probability < 1000 || probability >= ONE)
